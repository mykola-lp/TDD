...
function createApp(database: Database) {
  ...
  app.get("/prices", (req, res) => {
    ...
    const cost = calculateCost(age, type, date1, baseCost, date1);
    res.json({ cost });
  });
  ...
  function calculateCost(age: number | undefined, type: string, date: Date | Temporal.PlainDate | undefined, baseCost: number, date1: Temporal.PlainDate | undefined) {
    if (type === "night") {
      return calculateCostForNightTicket(age, baseCost);
    } else {
      return calculateCostForDayTicket(age, date1, baseCost);
    }
  }
